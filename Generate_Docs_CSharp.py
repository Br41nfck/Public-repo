...
def clean_summary_text(text):
    ...
    # Удаляем <summary> и </summary> если они вдруг остались
    joined = re.sub(r'<\/?summary>', '', joined, flags=re.IGNORECASE).strip()
    return html.escape(joined)

# Извлечение summary по классу 
# ВАЖНО: summary должен стоять непосредственно перед объявлением класса!
def extract_class_summaries(file_content):
    ...
    for m in CLASS_RE.finditer(file_content):
        ...
        # Если summary не стоит прямо перед классом, он не будет найден!
        if class_summary:
...
# Новый парсер с расширенной информацией
def extract_code_summaries(file_content, filename):
    ...
    while i < len(lines):
        ...
        if line.startswith('///') and '<summary>' in line:
            ...
            param_docs = {}
            returns_doc = ''
            # Собираем все строки summary и <param>/<returns>
            while i < len(lines) and '///' in lines[i]:
                ...
                if '</summary>' in lines[i]:
                    break
                i += 1
            # После summary ищем ближайшее объявление
            j = i + 1
            while j < len(lines) and (lines[j].strip() == '' or lines[j].strip().startswith('///')):
                j += 1
            if j < len(lines):
                code_line = lines[j].strip()
                # Атрибуты (ищем над объявлением)
                attrs = []
                k = j-1
                ...
                # Тип объекта, имя, параметры, возвращаемое значение
                obj_type = None
                obj_name = None
                params = ''
                returns = ''
...
def write_file_summaries_html(docs_dir, cs_file, summaries):
    ...
    with open(os.path.join(docs_dir, html_name), 'w', encoding='utf-8') as f:
        ...
        if summaries:
            ...
            f.write('</div>')
        # Кнопки и поиск
        f.write('''<div style="margin:10px 0;"><input type="text" id="searchInput" placeholder="Поиск по таблице..." style="width:300px;"> 
...
def main():
    ...
    for cs_file in cs_files:
        ...
        if summaries:  # Только если есть summary
